...
<div id="editMode" style="width:610px;margin:0 auto;">
	<form action="" id="base" name="base" method="post">
		...
		<input type="hidden" id="BULLET_ID" name="BULLET_ID" value="${BULLET_ID}" />
		<input type="hidden" id="BULLET_BOUND" name="BULLET_BOUND" value="${BULLET_BOUND}" />
		<input type="hidden" id="CASE_TYPE" name="CASE_TYPE" value="${param.CASE_TYPE}" />
		<input type="hidden" id="HOUSE_IDS" name="HOUSE_IDS" value="${param.HOUSING_IDS}" />
			...
			<ul style="width:610px;">
				<li class="border1px">
					<table width="100%" border="0" cellspacing="0" cellpadding="0" style="height:23px;padding-top:1px;">
						<tr>
							<td width="50"> 主 题：</td>
							<td>
								<input name="BULLET_SUBJECT" type="text" placeholder="标题的长度限制为 30 汉字" value="${bulletinInfo.values.BULLET_SUBJECT}" class="ipt-textfield"	id="topic" /></td>
							<td width="75" align="right">
								<a onfocus="this.blur();" onclick="${empty BULLET_ID ? 'showVote();' : ';'}return false;" id="vote" href="javascript:void(-1);" class="btn_b65 bt-blue-l">
				...
				<li>
					...
					<c:if test="${operator.operatorHT['SUB_VERSION'] ne 1 or operator.operatorHT['CITY_EXCHANGES_FLAG'] eq 1}">
						<label ${not empty BULLET_ID?'title="范围不可修改"':''} style="${ALLOW_CITY_BBS eq 0?'display:none;':''}">
							<input showText="同城交流" hidefocus type="checkbox"	${BULLET_BOUND eq 2 ? 'checked=checked' : ''} name="checkbox" value="2" onchange="choose(this)" id="checkbox"/>
						</label>
						<label ${not empty BULLET_ID?'title="范围不可修改"':''} style="${ALLOW_CITY_BBS eq 0?'display:none;':''}">
							<input showText="经纪天地" hidefocus type="checkbox"	${BULLET_BOUND eq 3 ? 'checked=checked' : ''} name="checkbox"	value="3" onchange="choose(this)" id="checkbox2"/>
						</label>
					</c:if>
					<c:if test="${operator.operatorHT['COMP_MODEL'] eq 2 and operator.operatorHT['COMP_ATTR'] eq 1 and PUB_BULLETIN_INFO_OPER}">
						<label ${not empty BULLET_ID?'title="范围不可修改"':''} style="${ALLOW_CITY_BBS eq 0?'display:none;':''}">
							<input showText="加&nbsp;盟&nbsp;圈" hidefocus type="checkbox"	${BULLET_BOUND eq 7 ? 'checked=checked' : ''} name="checkbox"	value="7" onchange="choose(this)" id="checkbox7"/>
						</label>
					</c:if>
					<!--纯精英版没有公司和本店-->
					<c:if test="${operator.operatorHT['DEPT_PERSONAL'] ne 0}">
						<c:if test="${operator.operatorHT['COMP_TYPE'] ne '2'}">
							<label ${not empty BULLET_ID?'title="范围不可修改"':''}>
								<input showText="本店信息" hidefocus type="checkbox" name="checkbox" value="0"	${BULLET_BOUND eq 0 ? 'checked=checked' : ''} id="checkbox4" onchange="choose(this)"/>
							</label>
						</c:if>
						<label ${not empty BULLET_ID?'title="范围不可修改"':''}>
							<input showText="公司信息" hidefocus type="checkbox"	name="checkbox"	${BULLET_BOUND eq 1 ? 'checked=checked' : ''} value="1" id="checkbox3" onchange="choose(this)"/>
						</label>
					</c:if>
				</li>
				<li style="padding:0;">
					...
					<div class="fileItem">
						...
						<ul>
							<c:forEach items="${bulletinPsPOs}" var="bulletinPs" varStatus="status">
								<li fileName="${bulletinPs.FILE_NAME}" path="${bulletinPs.PS_PATH}" url="<%=FastdfsManager.getHttpAccessDomain()%>${bulletinPs.PS_PATH}" >
									<span class="file_icon"></span>
									<span class="file_name">${bulletinPs.FILE_NAME}</span>
									<c:if test="${canEdit}">
					...
					<tr>
						...
						<td align="right" id="normalSubmit">
							...
							<div style="float: left;">
								显示电话：<label hidefocus class="blue"><input hidefocus type="radio" name="showPhone1" id="showPhone1"  ${bulletinInfo.values.SHOW_TEL eq 0 or empty BULLET_ID ? 'checked="checked"' : ''} value="0" />是</label>
								<label hidefocus class="blue"><input hidefocus type="radio" name="showPhone1" id="showPhone0" ${bulletinInfo.values.SHOW_TEL eq 1 ? 'checked="checked"' : ''} value="1"/>否</label>
							</div>
						</td>
					</tr>
				</table>
						...
						<tr>
							...
							<td align="right" id="voteSubmit" width="300">
								...
								显示电话：<label hidefocus class="blue">
									<input hidefocus type="radio" name="showPhone" id="showPhone1" ${bulletinInfo.values.SHOW_TEL eq 0 or empty BULLET_ID ? 'checked="checked"' : ''}  value="0" />是</label>
									<label hidefocus class="blue">
										<input hidefocus type="radio" name="showPhone" id="showPhone0" ${bulletinInfo.values.SHOW_TEL eq 1 ? 'checked="checked"' : ''} value="1"/>否
									</label>
								</div>
							</td>
						</tr>
...
<div id="initContent" style="display: none;">${bulletinInfo.values.BULLET_CONTENT}</div>
	...
	<c:if test="${not empty houseList}">
		<table id='99' border="0" cellpadding="0" cellspacing="0" class="tableCss" summary="房源列表">
			<tr>
				...
				<th style='width:80px;'>${param.CASE_TYPE eq 1 ? '价格' : '租金'}</th>
				<th colspan="2">查看</th>
			</tr>
			<c:forEach items="${houseList}" var="houseInfo" varStatus="status">
				<tr class="${status.index % 2 eq 0 ? 'trEven' : 'trOdd'}" id='fun${houseInfo.values.ID}'>
					<td>${houseInfo.values.REGION_NAME}</td>
					<td>${houseInfo.values.BUILD_NAME}</td>
					<td>${houseInfo.values.CASE_FLOOR}/${houseInfo.values.CASE_FLOORS}</td>
					<td>${houseInfo.values.CASE_ROOM}</td>
					<td>${pageUtil:getDicMsg('HOUSE_FITMENT',houseInfo.values.CASE_FITMENT)}</td>
					<td><fmt:formatNumber value="${houseInfo.values.CASE_AREA}" type="number" maxFractionDigits="0" pattern="##0.#"/>m<sup>2</sup></td>
					<td>
						<fmt:formatNumber value="${houseInfo.values.CASE_TOTAL_PRICE}" type="number" maxFractionDigits="0" pattern="##0.#"/>
						${empty houseInfo.values.PRICE_UNIT ? '万' : pageUtil:getDicMsg('PRICE_UNIT',houseInfo.values.PRICE_UNIT)}
					</td>
					<c:choose>
						<c:when test="${not empty houseInfo.values.MAP and houseInfo.values.MAP > 0}">
							<td><a id='showImg_${houseInfo.values.ID}' href='javascript:void(-1);' onclick="showHouseInfo('${houseInfo.values.ID}','${param.CASE_TYPE}','99',this.parentNode.parentNode.rowIndex,this,'${houseInfo.values.COMP_NO}','${houseInfo.values.DEPT_ID}','${houseInfo.values.USER_ID}','${houseInfo.values.picList}');return false;">图</a></td>
						</c:when>
						<c:otherwise>
							<td></td>
						</c:otherwise>
					...
					<c:choose>
						<c:when test="${not empty houseInfo.values.CHARACT}">
							<td><a id="showTxt_${houseInfo.values.ID}" href="javascript:void(-1);" onclick="showHouseInfo('${houseInfo.values.ID}','${houseInfo.values.CHARACT}','99',this.parentNode.parentNode.rowIndex,this,'${houseInfo.values.COMP_NO}','${houseInfo.values.DEPT_ID}','${houseInfo.values.USER_ID}');return false;">文</a></td>
						</c:when>
						<c:otherwise>
							<td></td>
						</c:otherwise>
...
<script type="text/javascript">
	var insertHouseInfoPath = "${SITE_ROOT_URL}/web/bulletin/insertHouseInfo?CLIENTKEY=${CLIENTKEY}&mtyefmupn"+new Date().getMilliseconds()+"="+Math.random(); //插入房源的页面路径
	var editor;
	KindEditor.ready(function(K) {
		...
		editor = K.create('#cb_ke_content', {
			...
			cssPath : '${SITE_ROOT}/publicJs/kindeditor/v4.1.7/themes/myfun7.css'
		});
		//如果是编辑 则初始化内容
		if($.trim($("#BULLET_ID").val()) != ""){
			editor.html('${pageUtil:reaplaceAll(bulletinInfo.values.BULLET_CONTENT,"\\n|\\r","")}');
		}
	...
	function preview(){
		if(checkSubmit()){
			...
			document.getElementById("previewFrame").src = "${SITE_ROOT_URL}/web/bulletin/preview?" + param.join("&");
		}
	}
	/**
	 * 预览页加载完调用的方法，由预览页调用
