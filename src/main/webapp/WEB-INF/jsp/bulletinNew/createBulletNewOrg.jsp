					...
					<div style="margin-left: 70px;padding-top: 6px;">
						<select lay-verify="required" id="bulletBounds" style="width: 100px;font-size: 12px;">
							<c:if test="${partnerFlag eq 1}">
								<option value="-1" ${bulletinInfo.organizationDefinitionId eq -1 ? 'selected' : ''}>加盟商</option>
							</c:if>
							<c:if test="${compFlag eq 1}">
								<option value="0" ${bulletinInfo.organizationDefinitionId eq 0 ? 'selected' : ''}>全公司</option>
								<option value="1" ${bulletinInfo.organizationDefinitionId eq 1 ? 'selected' : ''}>本组织</option>
							</c:if>
							<c:if test="${compFlag eq 0 && orgFlag eq 1}">
								<option value="1" ${bulletinInfo.organizationDefinitionId eq 1 ? 'selected' : ''}>本组织</option>
							</c:if>
						</select>
					</div>
