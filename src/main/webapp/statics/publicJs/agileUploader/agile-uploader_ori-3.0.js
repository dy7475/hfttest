(function($) {
	...
	*/
	...
	$.fn.agileUploaderClearTd = function(tdObj) {
		$(tdObj).addClass("only").removeClass("hasphoto").find(".fixtop").empty();
	...
	*/
	$.fn.agileUploader = function(options) {
		$("#agileUploaderFileList").find("li").each(function(){
			if($(this).attr("placeholded")==undefined || $(this).attr("placeholded")=="0")tdArr.push($(this).addClass("only").removeClass("hasphoto"));
		});
		var photoJson = options.photoJson;
		var _transdata = options.transdata;
		$(photoJson).each(function(i,data){
			...
			$("#remove-"+data.PHOTO_ID).click(function(){
				confirm("确定要删除该图片吗？", function(){
					$(this).unbind('click');
					var newData = data;
					var transData = {action:"remove",PHOTO_ID:newData.PHOTO_ID,IS_TOP:newData.IS_TOP,CASE_TYPE:newData.CASE_TYPE,
							...
							isPublish:newData.isPublish};
					$.extend(transData,_transdata||{});
					$.get(submitUrl,transData,function(data){
		...
		$(tdArr).each(function(){$(this).find(".agileUploaderFilePreview").prepend("<img src='images/default.jpg'/>")});
