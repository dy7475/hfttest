(function($) {
	...
	*/
	...
	$.fn.agileUploaderClearTd = function(tdObj) {
		$(tdObj).addClass("only").removeClass("showLabel hasphoto buildIMG sosoIMG").find(".fixtop").empty();
	...
	*/
	$.fn.agileUploader = function(options) {
		$("#agileUploaderFileList").find("li").each(function(){
		...
		$(photoJson).each(function(i,data){
			...
			tdObj = tdArr.shift();
			$(tdObj).attr("id","id-"+ data.PHOTO_ID).removeClass("only").addClass("hasphoto").find(".agileUploaderRemoveFile").html('<a href="javascript:void(0)" hideFocus class="removebtn" id="remove-' + data.PHOTO_ID + '" title="从上传列表中删除"></a>');
			if(data.IS_TOP == 1){
				tdObj.addClass("showLabel");
				currFirstLabelTd = tdObj.attr("id");
			}
			$(tdObj).find(".agileUploaderFilePreview").prepend("<img src='"+data.addr+"'/>");
			if(!!data.from && data.from=="SOSO"){
				//由于不同的需求，在页面初始化完成后绑定具体事件
				$("#id-"+data.PHOTO_ID).attr("imgsrc",data.src).addClass("sosoIMG");
			}else{
	 ...
	 * 选择楼盘图片后设置页面预览
	 */
	$.fn.agileSetBuildPic = function(type,id,addr){
		//console.log(type+"-"+id+"-"+addr);
		var tdArr;
		if(type==1){
			tdArr = tdArr1;
		}else if(type==2){
		...
		var buildAddrArr = (function(){
			var arr = [];
			$(".buildIMG").each(function(){
				arr.push($(this).attr("imgsrc"));
			});
			return arr.join(";");
		})();
		var data = addr;
		var preId = "b"+ type+"-"+id;
		if(buildAddrArr.indexOf(data)!=-1)return true;
		var picAddr = top.docPath + data;
		//console.log(top.docPath+"--------");
		var tdObj = tdArr.shift();
		$(tdObj).attr("id","id-"+preId).removeClass("only").attr("imgsrc",data).addClass("buildIMG hasphoto").find(".agileUploaderRemoveFile").html('<a href="javascript:void(0)" hideFocus class="removebtn" id="remove-' + preId + '" title="从上传列表中删除"></a>');
		$(tdObj).find(".agileUploaderFilePreview").prepend("<img src='"+picAddr+"'/>");
		$("#remove-"+preId).click(function(){
			var tdObj = $('#id-'+preId);
			var imgsrc = tdObj.attr("imgsrc");
			var $p_filestr_build = $("#BUILD_PHOTO",parent.document);
			var tfileArr = $p_filestr_build.val().split(";");
			tfileArr = $.grep(tfileArr,function(val,key){
				return (val!=imgsrc);
			})
			$p_filestr_build.val(tfileArr.join(";"));
			$(this).unbind('click');
			$("#id-"+preId +" a").unbind("click");
			$(tdObj).removeAttr("imgsrc");
			$.fn.agileUploaderClearTd(tdObj);
			refreshParent();
		});
		$("#id-"+preId +" .setCover").data("arr",["", preId,""]).click(function(){
			confirm("确定将房源首图换为该图片吗？", function(){
				$(this).unbind('click');
				var tdObj = $('#id-'+preId);
				$.fn.agileUploaderSetCover(tdObj);
		...
		if($(".showLabel").length==0)$(".hasphoto .setCover:eq(0)").data("clickType","auto").trigger("click");
	}
	$.fn.agileClearBuildPic = function(type,id){
		var preId = "b"+ type+"-"+id;
		var tdObj = $("#id-"+preId);
		$.fn.agileUploaderClearTd(tdObj);
	}
