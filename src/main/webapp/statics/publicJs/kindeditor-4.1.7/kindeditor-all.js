...
K.options = {
	designMode : true,
	custImage : false,//jsg 是否自定义上传图片 点击上传图片按钮直接选择文件
	...
	hiddenItems : ['multiimage', 'flash', 'media'],
	colorTable : [
...
KEditor.prototype = {
	...
	create : function() {
		...
		K.each(self.items, function(i, name) {
			if(_isArray(self.hiddenItems) && (_inArray(name,self.hiddenItems) >= 0)){
				return;
			}else if (name == '|') {
				htmlList.push('<span class="ke-inline-block ke-separator"></span>');
			} else if (name == '/') {
				htmlList.push('<div class="ke-hr"></div>');
			} else if(name == "image" && self.options.custImage){
				var targetId = 'kindeditor_upload_iframe_' + new Date().getTime();
				//jsg自定义上传图片控件
				htmlList.push('<span class="ke-outline" data-name="' + name + '" title="' + self.lang(name) + '" unselectable="on">');
				htmlList.push('	<span style="position: relative;" class="ke-toolbar-icon ke-toolbar-icon-url ke-icon-' + name + '" unselectable="on">');
				htmlList.push('	<iframe name="' + targetId + '" style="display:none;"></iframe>');
				htmlList.push('<form></form>');
				htmlList.push('	<form action="' +self.uploadJson+ '" target="'+targetId+'" method="post" enctype="multipart/form-data">');
				htmlList.push('		<input type="file" name="file" style="position: absolute;filter: alpha(opacity=30);opacity:0.2;left:-5px;width: 100%;height: 100%;" onchange="custUploadImage(this,\''+targetId+'\')"/>');
				htmlList.push('	</form>');
				htmlList.push('	</span>');
				htmlList.push('</span>');
 ...
 * jsg 自定义上传图片
 */
function custUploadImage(obj, targetId){
	var iframe = window.KindEditor('iframe[name="' + targetId + '"]');
	iframe.bind('load', function() {
		iframe.unbind();
		var doc = window.KindEditor.iframeDoc(iframe),
			pre = doc.getElementsByTagName('pre')[0],
			str = '', data;
		if (pre) {
			str = pre.innerHTML;
		...
		str = $.trim(window.KindEditor.unescape(str));
		iframe[0].src = 'javascript:false';
		try {
			data = eval("(" +str+ ")");
		} catch (e) {
			alert(window.KindEditor.lang('uploadError'))
		}
		if (data) {
			if (data.error === 0) {
				var url = data.url;
				if (window.KindEditor.formatUploadUrl) {
					url = window.KindEditor.formatUrl(url, 'absolute');
				}
				window.editor.exec('insertimage', url);
				//alert(window.KindEditor.lang('uploadSuccess'));
			} else {
				alert(data.message);
			}
		}
