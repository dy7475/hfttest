...
$(document).ready(function(){
	...
	payConfig.cashMoney = 0;//cashMoney;
	payConfig.shareMoney = 0;//shareMoney;
	...
	$("#payForName").text("购买软件");
	$("#payForUseage").html("*云存储安全存放数据，银行级加密备份，为你节省成本<br>*网络房源搜索引擎，让您1分钟获取房源一键登记，为你节省时间<br>*网站广告群发利器，1分钟完成任务、后续更新，管理更方便<br>*为ERP和APP全城联网、智能匹配、信息推送、视频微店等O2O服务功能，全方位为您提供房产O2O解决方案");
	...
	if(bindOpenWeixin == 1 && weiContractNo != undefined && payConfig.deptFlag == 1){
		...
		if(payConfig.deptType == 1){
			...
			$("#payInfoBox .text").html("购买好房通Pro.+企业微店");
		}
	...
	$("#payInfoBox .price").text(payConfig.needPrice + "元");// 所需支付总金额
