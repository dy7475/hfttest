...
var payConfig = {
	...
	returnUrl : "http://erpweb.myfun7.com/erpWeb/usercenter/payment/superHouse/result?CLIENTKEY="+clientKey,
	// jsg
...
//	returnUrl : "http://192.168.5.172:8080/erpWeb/usercenter/payment/superHouse/result?CLIENTKEY="+clientKey,
	// 101
...
//	returnUrl : "http://192.168.11.101:8080/erpWeb/usercenter/payment/superHouse/result?CLIENTKEY="+clientKey,
};

$(document).ready(function(){
	initUrlConfig("usercenter/payment/superHouse/result?CLIENTKEY="+clientKey);
	...
	$("#payForName").text("历史房源库");
	$("#payForUseage").html("1、海量全城历史业主房源资料，深度发掘潜在用户<br>2、每店每年可查看10000条");
	$("#payForName").css('background', 'url("'+statics+'ucenter/superHouse/img/superHouse.png") no-repeat 12px center');
	...
	ajaxTemp(payConfig.crmWebUrl + "crmOpenApi/contractOpenApi/deptSoftRule", "POST", "json", {
			...
			if (validateCrmResult(data, "", 1)) {
				...
				payConfig.unitPrice = data.data.custLevelPrice;
				var endTimeNow = data.data.custEndTime;// 超级房源库到期时间
				var html = "";
				var extJson = [1, 2, 3];
				for (var i = 0; i < extJson.length; i++) {
					...
					var pirce = year * data.data.custLevelPrice;
					// 计算到期时间
...
function calculatePrice() {
	...
	$("#payInfoDiv .info").text("超级房源库"+year+"年");
	payConfig.ptId = "";
