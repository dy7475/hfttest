...
import com.myfun.repository.reportdb.po.ReportFunRentCustomerMin;
...
public interface ReportFunRentCustomerMinMapper extends BaseMapper<ReportFunRentCustomerMin, Long> {
    /**
	 * @mbggenerated
	 */
	int countByExample(ReportFunRentCustomerMinExample example);

	/**
	 * @mbggenerated
	 */
	int deleteByExample(ReportFunRentCustomerMinExample example);

	/**
	 * @mbggenerated
	 */
	List<ReportFunRentCustomerMin> selectByExample(ReportFunRentCustomerMinExample example);

	/**
	 * @mbggenerated
	 */
	int updateByExampleSelective(@Param("record") ReportFunRentCustomerMin record,
			@Param("example") ReportFunRentCustomerMinExample example);

	/**
	 * @mbggenerated
	 */
	int updateByExample(@Param("record") ReportFunRentCustomerMin record,
			@Param("example") ReportFunRentCustomerMinExample example);
	 ...
	 * @return
	 */
	@SelectProvider(method = "getStoreDataAnalysisByUseage", type = ReportFunRentCustomerMinSqlProvider.class)
	 ...
	 * @return
	 */
	@SelectProvider(method = "getBuildSupplyDemandComparisonForRent", type = ReportFunRentCustomerMinSqlProvider.class)
	@ResultType(Map.class)
	List<Map<String, Object>> getBuildSupplyDemandComparisonForRent(@Param("shardCityId")Integer cityId, @Param("param")Map<String, Object> param);

	List<Map<String,Object>> getCompRentCustomerCountByDate(@Param("param")AnalysisForWebParam param);

	class ReportFunRentCustomerMinSqlProvider {
		public String getStoreDataAnalysisByUseage(Map<String,Object> pMap) throws Exception{
			...
			sqlBuilder.append(" select top 10 count(1) counts,c.SOURCE_ID byName from FUN_RENT_CUSTOMER_MIN a ");
			sqlBuilder.append(" join FUN_CUST_DEMAND_SPLIT_4 b on b.CASE_TYPE=4 and a.RENT_CUST_ID = b.CASE_ID  and a.CITY_ID = b.CITY_ID");
			sqlBuilder.append(" join FUN_CUST_DEMAND_SPLIT_5 c on c.CASE_TYPE=4 and a.RENT_CUST_ID = c.CASE_ID  and a.CITY_ID = b.CITY_ID");
			...
			if("1".equals(statusType)){
				sqlBuilder.append(" AND a.RENT_CUST_STATUS IN (1,2,4) ");
			}
			if("1".equals(useage)){
			...
			sqlBuilder.append(" group by c.SOURCE_ID order by count(1) desc");
			return sqlBuilder.toString();
		}
		public String getBuildSupplyDemandComparisonForRent(Map<String,Object> pMap) throws Exception{
			BaseMap param = (BaseMap) pMap.get("param");
			String statusType = param.getString("statusType");
			//String useage = param.getString("useage");
			String serchRange = param.getString("serchRange");
			...
			StringBuilder sqlBuilder = new StringBuilder();
			sqlBuilder.append(" select top 4 count(1) sectionCustCount,c.SOURCE_ID sectionId from FUN_RENT_CUSTOMER_MIN a ");
			sqlBuilder.append(" join FUN_CUST_DEMAND_SPLIT_2 c on c.CASE_TYPE=4 and a.RENT_CUST_ID = c.CASE_ID and a.CITY_ID = c.CITY_ID");
			sqlBuilder.append(" where a.COMP_ID=#{param.compId} and a.CITY_ID=#{shardCityId} ");
			sqlBuilder.append(" AND a."+serchRange+"=#{param.serchRangeId}");
