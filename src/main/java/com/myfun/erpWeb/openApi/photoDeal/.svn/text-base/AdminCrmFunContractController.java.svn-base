 ...
 */
@Controller
@RequestMapping(value = "/openApi/adminCrmFunContractPhtot")
public class AdminCrmFunContractController extends BaseController{
...
//		开启一个读线程
		new AdminCrmFunContractControllerThread().start();
	 ...
	 *
	 */
	public class AdminCrmFunContractControllerThread extends Thread {
		...
		public void run() {
			while (true) {
				try {
					...
					adminCrmFunContractMapper.insertForFastDfs(pMap);
		...
		public void run() {
			try {
				for (int i = 0; i < dealSize; i++) {
					...
					// 分页读取 免得搞死数据库
					AdminCrmFunContractExample adminCrmFunContractExample = new AdminCrmFunContractExample();
					adminCrmFunContractExample.createCriteria().andContractPathIsNotNull().andContractPathLike("contract%");

					Integer totalCount = adminCrmFunContractMapper.countByExample(adminCrmFunContractExample);
					adminCrmFunContractExample.setOrderByClause(" CONTRACT_ID ASC");
					...
					for (Integer j = 1; j <= totalPage ; j++ ) {
						...
						List<AdminCrmFunContract> videoList =  adminCrmFunContractMapper.selectByExample(adminCrmFunContractExample);
						// 没有数据时退出循环
						...
						for (AdminCrmFunContract adminCrmFunContract : videoList) {

							StringBuilder stringBuilder = new StringBuilder("");
							if(StringUtils.isNotBlank(adminCrmFunContract.getContractPath())) {
								// 拷贝文件
								String[] paths = adminCrmFunContract.getContractPath().split("\\|");
								for (String path : paths) {
									String contractPath = uploadFile(path);
									stringBuilder.append(contractPath).append("|");
								}
							}

							Map<String, Object> fastdfsParamMap = new HashMap<>();
							fastdfsParamMap.put("contractId", adminCrmFunContract.getContractId());

							fastdfsParamMap.put("contractPathOld", adminCrmFunContract.getContractPath());
							fastdfsParamMap.put("contractPathNew", stringBuilder.toString());
