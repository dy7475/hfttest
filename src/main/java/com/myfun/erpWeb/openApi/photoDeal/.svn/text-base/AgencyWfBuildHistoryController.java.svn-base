...
import com.myfun.repository.agencydb.dao.AgencyWfBuildHistoryMapper;
 ...
 */
@Controller
@RequestMapping(value = "/openApi/agencyWfBuildHistory")
public class AgencyWfBuildHistoryController extends BaseController{
	private static final Logger logger = LoggerFactory.getLogger(PhotoDealController.class);
	
	@Autowired
	AgencyWfBuildHistoryMapper agencyWfBuildHistoryMapper;
		...
		public void run() {
			try {
				for (int i = 0; i < dealSize; i++) {
					...
					// 只有几条 不用分页处理
					AgencyWfBuildHistoryExample param = new AgencyWfBuildHistoryExample();
					param.createCriteria().andBuildPhotoIsNotNull().andBuildPhotoNotLike("group%");
					List<AgencyWfBuildHistory> videoList =  agencyWfBuildHistoryMapper.selectByExample(param);
					// 没有数据时退出循环
					...
					for (AgencyWfBuildHistory agencyWfBuildHistory : videoList) {
						// 拷贝文件
						String newPhotoAddr = uploadFile(agencyWfBuildHistory.getBuildPhoto());
						// 处理的有文件
						if(StringUtils.isNotBlank(newPhotoAddr)) {
							Map<String, Object> fastdfsParamMap = new HashMap<String, Object>();
							fastdfsParamMap.put("wfId", agencyWfBuildHistory.getId());
							fastdfsParamMap.put("buildPhotoOld", agencyWfBuildHistory.getBuildPhoto());
							fastdfsParamMap.put("buildPhotoNew", newPhotoAddr);
					...
					// 写入新表
					agencyWfBuildHistoryMapper.insertForFastDfs(fastdfsFileList);
				}
