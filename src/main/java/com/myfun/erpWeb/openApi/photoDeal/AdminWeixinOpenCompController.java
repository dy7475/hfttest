 ...
 */
@Controller
@RequestMapping(value = "/openApi/adminWeixinOpenCompPhoto")
public class AdminWeixinOpenCompController extends BaseController{
	private static final Logger logger = LoggerFactory.getLogger(PhotoDealController.class);
	
	@Autowired
	AdminWeixinOpenCompMapper adminWeixinOpenCompMapper;
		...
		public void run() {
			while (true) {
				try {
					...
					adminWeixinOpenCompMapper.insertForFastDfs(pMap);
		...
		public void run() {
			try {
				...
				for (int i = 0; i < dealSize; i++) {
					...
					// 分页读取 免得搞死数据库
					AdminWeixinOpenCompExample adminWeixinOpenCompExample = new AdminWeixinOpenCompExample();
					adminWeixinOpenCompExample.createCriteria().andWeixinCompLogoNotLike("group%").andWeixinCompLogoIsNotNull();

					Integer totalCount = adminWeixinOpenCompMapper.countByExample(adminWeixinOpenCompExample);
					adminWeixinOpenCompExample.setOrderByClause(" WEIXIN_COMP_ID ASC");
					...
					for (Integer j = 1; j <= totalPage ; j++ ) {
						...
						List<AdminWeixinOpenComp> videoList =  adminWeixinOpenCompMapper.selectByExample(adminWeixinOpenCompExample);
						// 没有数据时退出循环
						...
						for (AdminWeixinOpenComp adminWeixinOpenComp : videoList) {

							String weixinCompLog = "";
							if(StringUtils.isNotBlank(adminWeixinOpenComp.getWeixinCompLogo())) {
								// 拷贝文件
								weixinCompLog = uploadFile(adminWeixinOpenComp.getWeixinCompLogo());
							}

							Map<String, Object> fastdfsParamMap = new HashMap<>();
							fastdfsParamMap.put("weiXinCompId", adminWeixinOpenComp.getWeixinCompId());

							fastdfsParamMap.put("weiXinCompLogOld", adminWeixinOpenComp.getWeixinCompLogo());
							fastdfsParamMap.put("weiXinCompLogNew", weixinCompLog);
