...
import com.myfun.repository.admindb.dao.AdminWMicroshopAttachMapper;
import com.myfun.repository.admindb.dao.AdminWMicroshopMessageMapper;
import com.myfun.repository.admindb.dao.AdminWMicroshopNewsmsgMapper;
 ...
 */
@Controller
@RequestMapping(value = "/openApi/AdminWMicroshopNewsmsgPhoto")
public class AdminWMicroshopMessageController extends BaseController{
	private static final Logger logger = LoggerFactory.getLogger(PhotoDealController.class);
	
	@Autowired
	AdminWMicroshopNewsmsgMapper adminWMicroshopNewsmsgMapper;
		...
		public void run() {
			while (true) {
				try {
					...
					adminWMicroshopNewsmsgMapper.insertForFastDfs(pMap);
		...
		public void run() {
			try {
				for (int i = 0; i < dealSize; i++) {
					...
					// 分页读取 免得搞死数据库
					AdminWMicroshopNewsmsgExample adminWMicroshopMessageExample = new AdminWMicroshopNewsmsgExample();
					adminWMicroshopMessageExample.createCriteria().andPicurlNotLike("group%").andPicurlIsNotNull();

					Integer totalCount = adminWMicroshopNewsmsgMapper.countByExample(adminWMicroshopMessageExample);
					adminWMicroshopMessageExample.setOrderByClause(" id ASC");
					...
					for (Integer j = 1; j <= totalPage ; j++ ) {
						...
						List<AdminWMicroshopNewsmsg> videoList =  adminWMicroshopNewsmsgMapper.selectByExample(adminWMicroshopMessageExample);
						// 没有数据时退出循环
						...
						for (AdminWMicroshopNewsmsg adminWMicroshopNewsmsg : videoList) {

							String picUrl = "";
							if(StringUtils.isNotBlank(adminWMicroshopNewsmsg.getPicurl())) {
								// 拷贝文件
								picUrl = uploadFile(adminWMicroshopNewsmsg.getPicurl());
							}
							...
							fastdfsParamMap.put("wmId", adminWMicroshopNewsmsg.getId());

							fastdfsParamMap.put("picUrlOld", adminWMicroshopNewsmsg.getPicurl());
							fastdfsParamMap.put("picUrlNew", picUrl);
...
//						adminFunArchiveMapper.insertForFastDfs(fastdfsFileList);
	...
	private static String uploadFile(String fileAddr) {
		try{
			...
			fileAddr = "http://pic.myfun7.cn/weixin/"+fileAddr;
