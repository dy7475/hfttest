 ...
 */
@Controller
@RequestMapping(value = "/openApi/photoDeal/erpTrackPhoto")
public class ErpTrackPhotoController extends BaseController{
	private static final Logger logger = LoggerFactory.getLogger(ErpTrackPhotoController.class);
	
	@Autowired
	ErpTrackPhotoMapper erpTrackPhotoMapper;
	@Autowired
...
//		dealThreadCount.put("hft_erpdb_zz", 20);
		...
		public void run() {
			try {
				while(true) {
					...
					List<ErpTrackPhoto> photoList = null;
					synchronized(ErpTrackPhotoController.class){
					...
					for (ErpTrackPhoto erpTrackPhoto : photoList) {
						String photoAddr = "http://pic.myfun7.com/pic/hftpic/house/" + erpTrackPhoto.getPhotoAddr();
						FileUploadResult uploadResult = FileUtil.fileUpload(photoAddr, AppConfig.getFileWebUrl(),"0");
						if(null != uploadResult) {
							Map<String, Object> pMap = new HashMap<>();
							pMap.put("photoAddr", uploadResult.getFilePath());
							pMap.put("photoId", erpTrackPhoto.getPhotoId());
					...
					if(fastdfsPhotoList.isEmpty()) {
						...
						erpTrackPhotoMapper.updatePhotoAddrForList(shardDbName, fastdfsPhotoList);
					}
				}
	...
	public List<ErpTrackPhoto> getPhotoListOrderByPhotoIdDesc(String shardDbName) {
		Integer photoId = erpTrackPhotoMapper.selectLastPhotoId(shardDbName);
		List<ErpTrackPhoto> photoList = erpTrackPhotoMapper.getPhotoListOrderByPhotoIdDesc(shardDbName, photoId);
		if(photoList == null || photoList.size() <= 0) {
			return null;
		}
		List<Map<String, Object>> fastdfsPhotoList = new LinkedList<>();
		for (ErpTrackPhoto erpTrackPhoto : photoList) {
			Map<String, Object> pMap = new HashMap<>();
			pMap.put("photoId", erpTrackPhoto.getPhotoId());
			pMap.put("oldAddr", erpTrackPhoto.getPhotoAddr());
			fastdfsPhotoList.add(pMap);
		}
		erpTrackPhotoMapper.insertList(shardDbName, fastdfsPhotoList);
		return photoList;
	}
}
