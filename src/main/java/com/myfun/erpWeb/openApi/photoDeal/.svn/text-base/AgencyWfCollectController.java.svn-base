 ...
 */
@Controller
@RequestMapping(value = "/openApi/agencyWfCollectPhoto")
public class AgencyWfCollectController extends BaseController{
	private static final Logger logger = LoggerFactory.getLogger(PhotoDealController.class);
	
	@Autowired
	AgencyWfCollectMapper agencyWfCollectMapper;
		...
		public void run() {
			while (true) {
				try {
					...
					agencyWfCollectMapper.insertForFastDfs(pMap);
		...
		public void run() {
			try {
				...
				for (int i = 0; i < dealSize; i++) {
					...
					// 分页读取 免得搞死数据库
					AgencyWfCollectExample agencyWfCollectExample = new AgencyWfCollectExample();
					agencyWfCollectExample.createCriteria().andThumbUrlIsNotNull().andThumbUrlNotLike("group%");

					Integer totalCount = agencyWfCollectMapper.countByExample(agencyWfCollectExample);
					agencyWfCollectExample.setOrderByClause(" ID ASC");
					...
					for (Integer j = 1; j <= totalPage ; j++ ) {
						...
						List<AgencyWfCollect> videoList =  agencyWfCollectMapper.selectByExample(agencyWfCollectExample);
						// 没有数据时退出循环
						...
						for (AgencyWfCollect agencyWfCollect : videoList) {

							String thumbUrl = "";
							if(StringUtils.isNotBlank(agencyWfCollect.getThumbUrl())) {
								// 拷贝文件
								thumbUrl = uploadFile(agencyWfCollect.getThumbUrl());
							}

							Map<String, Object> fastdfsParamMap = new HashMap<>();
							fastdfsParamMap.put("wfId", agencyWfCollect.getId());

							fastdfsParamMap.put("thumbUrlOld", agencyWfCollect.getThumbUrl());
							fastdfsParamMap.put("thumbUrlNew", thumbUrl);
...
//						adminFunArchiveMapper.insertForFastDfs(fastdfsFileList);
	...
	private static String uploadFile(String fileAddr) {
		try{
			...
			String picDomain = "";
			if(fileAddr.startsWith("thumb")) {
				fileAddr = "http://img01-uat.m1200.com.cn/"+fileAddr;
			}else {
				fileAddr = "http://pic.myfun7.com/pic/hftpic/net/"+fileAddr;
			}
