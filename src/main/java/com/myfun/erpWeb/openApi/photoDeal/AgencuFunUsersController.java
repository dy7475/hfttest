 ...
 */
@Controller
@RequestMapping(value = "/openApi/AgencyFunUsersPhoto")
public class AgencuFunUsersController extends BaseController{
		...
		public void run() {
			while (true) {
				try {
					...
					agencyFunUsersMapper.insertForFastDfs(pMap);
		...
		public void run() {
			try {
				for (int i = 0; i < dealSize; i++) {
					...
					// 分页读取 免得搞死数据库
					AgencyFunUsersExample agencyFunUsersExample = new AgencyFunUsersExample();
					agencyFunUsersExample.createCriteria().andUserPhotoIsNotNull().andUserPhotoNotLike("group%");
					Integer totalCount = agencyFunUsersMapper.countByExample(agencyFunUsersExample);
					agencyFunUsersExample.setOrderByClause(" USER_ID ASC");
					...
					for (Integer j = 1; j <= totalPage ; j++ ) {
						...
						List<AgencyFunUsers> videoList =  agencyFunUsersMapper.selectByExample(agencyFunUsersExample);
						// 没有数据时退出循环
						...
						for (AgencyFunUsers agencyFunUsers : videoList) {

							String userPhoto = "";
							if(StringUtils.isNotBlank(agencyFunUsers.getUserPhoto())) {
								// 拷贝文件
								userPhoto = uploadFile(agencyFunUsers.getUserPhoto());
							}

							Map<String, Object> fastdfsParamMap = new HashMap<>();
							fastdfsParamMap.put("userId", agencyFunUsers.getUserId());

							fastdfsParamMap.put("userPhotoOld", agencyFunUsers.getUserPhoto());
