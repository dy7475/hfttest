 ...
 */
@Controller
@RequestMapping(value = "/openApi/photoDeal/agencyBuildingPhotoDealController")
public class AgencyBuildingPhotoDealController extends BaseController {
	private static final Logger logger = LoggerFactory.getLogger(AgencyBuildingPhotoDealController.class);
			...
			while(true){
				try {
					...
					for (AgencyBuildingPhoto agencyBuildingPhoto : agencyBuildingPhotos) {
						...
						String newAddr = "";
						try {
							newAddr = FileUtil.uploadFileToFastdfs("/hftdata/static/pic/hftpic/house/", photoAddr);
						} catch (Exception e) {
							e.printStackTrace();
						}
