 ...
 */
@Controller
@RequestMapping(value = "/openApi/adminFunInvoiseEnclosure")
public class AdminFunInvoiseController extends BaseController{
	private static final Logger logger = LoggerFactory.getLogger(PhotoDealController.class);
	
	@Autowired
	AdminFunInvoiceEnclosureMapper adminFunInvoiceEnclosureMapper;
		...
		public void run() {
			try {
				...
				for (int i = 0; i < dealSize; i++) {
					...
					// 只有几条 不用分页处理
					AdminFunInvoiceEnclosure param = new AdminFunInvoiceEnclosure();
					param.setEsPath("invoice");
					List<AdminFunInvoiceEnclosure> videoList =  adminFunInvoiceEnclosureMapper.selectList(param);
					// 没有数据时退出循环
					...
					for (AdminFunInvoiceEnclosure adminFunInvoiceEnclosure : videoList) {
						// 拷贝文件
						String newPhotoAddr = uploadFile(adminFunInvoiceEnclosure.getEsPath());
						// 处理的有文件
						if(StringUtils.isNotBlank(newPhotoAddr)) {
							Map<String, Object> fastdfsParamMap = new HashMap<>();
							fastdfsParamMap.put("esId", adminFunInvoiceEnclosure.getEsId());
							fastdfsParamMap.put("pathOld", adminFunInvoiceEnclosure.getEsPath());
					...
					// 写入新表
					adminFunInvoiceEnclosureMapper.insertForFastDfs(fastdfsFileList);
				}
