...
import com.myfun.repository.admindb.po.AdminWMicroshopAttach;
import com.myfun.repository.admindb.po.AdminWMicroshopAttachExample;
 ...
 */
@Controller
@RequestMapping(value = "/openApi/AdminWMicroshopAttachPhoto")
public class AdminWMicroshopAttachController extends BaseController{
	private static final Logger logger = LoggerFactory.getLogger(PhotoDealController.class);
	
	@Autowired
	AdminWMicroshopAttachMapper adminWMicroshopAttachMapper;
		...
		public void run() {
			while (true) {
				try {
					...
					adminWMicroshopAttachMapper.insertForFastDfs(pMap);
		...
		public void run() {
			try {
				for (int i = 0; i < dealSize; i++) {
					...
					// 分页读取 免得搞死数据库
					AdminWMicroshopAttachExample adminWMicroshopAttachExample = new AdminWMicroshopAttachExample();
					adminWMicroshopAttachExample.createCriteria().andPathNotLike("group%").andPathIsNotNull();

					Integer totalCount = adminWMicroshopAttachMapper.countByExample(adminWMicroshopAttachExample);
					adminWMicroshopAttachExample.setOrderByClause(" id ASC");
					...
					for (Integer j = 1; j <= totalPage ; j++ ) {
						...
						List<AdminWMicroshopAttach> videoList =  adminWMicroshopAttachMapper.selectByExample(adminWMicroshopAttachExample);
						// 没有数据时退出循环
						...
						for (AdminWMicroshopAttach adminWMicroshopAttach : videoList) {

							String defaultImage = "";
							if(StringUtils.isNotBlank(adminWMicroshopAttach.getPath())) {
								// 拷贝文件
								defaultImage = uploadFile(adminWMicroshopAttach.getPath());
							}
							...
							fastdfsParamMap.put("wmId", adminWMicroshopAttach.getId());

							fastdfsParamMap.put("pathOld", adminWMicroshopAttach.getPath());
							fastdfsParamMap.put("pathNew", defaultImage);
