 ...
 */
@Controller
@RequestMapping(value = "/openApi/agencyBuildLayoutPhoto")
public class AgencyBuildLayoutController extends BaseController{
	...
	public class AdminFunArchiveControllerThread extends Thread {
		...
		public void run() {
			while (true) {
				try {
					while (pMap.size() < 100) {
						Map<String, Object> take = FUN_ARCHIVE_FAST_DFS_TMP.take();
						if(null == take || take.isEmpty()) {
							continue;
						}
						pMap.add(take);
					}
					agencyBuildLayoutMapper.insertForFastDfs(pMap);
		...
		public void run() {
			try {
				...
				for (int i = 0; i < dealSize; i++) {
					...
					// 分页读取 免得搞死数据库
					AgencyBuildLayoutExample agencyBuildLayoutExample = new AgencyBuildLayoutExample();
					agencyBuildLayoutExample.or().andLayoutPic1Like("agency%");
					agencyBuildLayoutExample.or().andLayoutPic2Like("agency%");

					Integer totalCount = agencyBuildLayoutMapper.countByExample(agencyBuildLayoutExample);
					agencyBuildLayoutExample.setOrderByClause(" LAYOUT_ID ASC");
					...
					for (Integer j = 1; j <= totalPage ; j++ ) {
						...
						List<AgencyBuildLayout> videoList =  agencyBuildLayoutMapper.selectByExample(agencyBuildLayoutExample);
						// 没有数据时退出循环
						...
						for (AgencyBuildLayout agencyBuildLayout : videoList) {

							String layoutPic1 = "";
							String layoutPic2 = "";
							if(StringUtils.isBlank(agencyBuildLayout.getLayoutPic1()) && StringUtils.isBlank(agencyBuildLayout.getLayoutPic2())) {
								continue;
							}
							if(StringUtils.isNotBlank(agencyBuildLayout.getLayoutPic1())) {
								// 拷贝文件
								layoutPic1 = uploadFile(agencyBuildLayout.getLayoutPic1());
							}
							if(StringUtils.isNotBlank(agencyBuildLayout.getLayoutPic2())) {
								// 拷贝文件
								layoutPic2 = uploadFile(agencyBuildLayout.getLayoutPic2());
							}

							Map<String, Object> fastdfsParamMap = new HashMap<>();
							fastdfsParamMap.put("layoutId", agencyBuildLayout.getLayoutId());

							fastdfsParamMap.put("layoutPic1Old", agencyBuildLayout.getLayoutPic1());
							fastdfsParamMap.put("layoutPic1New", layoutPic1);

							fastdfsParamMap.put("layoutPic2Old", agencyBuildLayout.getLayoutPic2());
							fastdfsParamMap.put("layoutPic2New", layoutPic2);
