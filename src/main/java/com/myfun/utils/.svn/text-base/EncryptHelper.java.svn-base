...
import com.myfun.utils.encrypt.KeyBean;
import com.myfun.utils.encrypt.RSA;
import com.myfun.utils.encrypt.XXTEA;
...
/**
 * Base64、SHA1（摘要）、MD5（摘要）、AES（对称）、 XXTEA（对称）加密算法
 * 
 * @ClassName: EncryptHelper
 * @Description: TODO
 * @author jiangsonggui
 * @date 2015年11月23日 上午11:03:39
 *
 */
public class EncryptHelper {
	...
	public static String encryptSHA1(String data) {
		try {
			...
			return ByteUtils.byte2HexString(digest.digest());
	...
	public static String encryptMD5(String data) {
		try {
			...
			return ByteUtils.byte2HexString(mdInst.digest());
		} catch (NoSuchAlgorithmException e) {
		}
		return null;
	}
	...
	public static String encryptAES(String content, String password) {
		try {
			...
			return ByteUtils.byte2HexString(result);
		} catch (Exception e) {
		}
		return null;
	}
	...
	public static String decryptAES(String content, String password) {
		return decryptAES(ByteUtils.hexString2Byte(content), password);
	}
	...
	public static String decryptBase64(String key) {
		...
		return ByteUtils.byte2HexString(Base64.decodeBase64(key));
	}
	...
	public static String encryptBase64(String key) {
		...
		return ByteUtils.byte2HexString(Base64.encodeBase64(key.getBytes()));
	}

	public static String enBase64(String key) {
		if (StringUtil.isEmpty(key)) {
			return key;
		}
		return new String(Base64.encodeBase64(key.getBytes()), Charset.forName("utf-8"));
	}

	public static String encryptXXTEA(String dataStr, String key) {
		return XXTEA.encrypt(dataStr, key);
	}

	public static String decryptXXTEA(String dataStr, String key) {
		return XXTEA.decrypt(dataStr, key);
	}
	 ...
	 * @return
	 */
	public static String encryptRsaByPrivateKey(String data, String privateKey) {
		try {
			byte[] enData = RSA.encryptByPrivateKey(data.getBytes(), privateKey);
			return ByteUtils.byte2HexString(enData);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	 ...
	 * @return
	 */
	public static String encryptRsaByPublicKey(String data, String publicKey) {
		try {
			byte[] enData = RSA.encryptByPublicKey(data.getBytes(), publicKey);
			return ByteUtils.byte2HexString(enData);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @return
	 */
	public static String decryptRsaByPrivateKey(String data, String privateKey) {
		try {
			byte[] deData = RSA.decryptByPrivateKey(ByteUtils.hexString2Byte(data), privateKey);
	...
	public static String decryptRsaByPublicKey(String data, String publicKey) {
		try {
			byte[] deData = RSA.decryptByPublicKey(ByteUtils.hexString2Byte(data), publicKey);
			return new String(deData);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static String getRsaPrivateKey(Map<String, Object> keyMap) {
		return RSA.getPrivateKey(keyMap);
	}
	 ...
	 * @return
	 */
	public static String encryptHftPassword(String key) {
		KeyBean bean = new KeyBean();
		return bean.getkeyBeanofStr(key);
	}
	...
	*************************************************/
	public static class getKeyBean {
		...
		*/
		public static String getkeyBeanofStr(String inbuf) {
			keyBeanInit();
			inbuf = "m%y#f^u@n)7" + inbuf + "7-n_u+f!y?m";
		...
		public static String getHftMd5(String inbuf){
			keyBeanInit();
			inbuf = "m%Y#m^D@5)" + inbuf + "f-^r_A+n!K?p#a(R&k";
			keyBeanUpdate(inbuf.getBytes(), inbuf.length());
			keyBeanFinal();
			digestHexStr = "";
			for (int i = 0; i < 16; i++) {
		...
		*/
		...
		函数由getkeyBeanofStr调用，调用之前需要调用keyBeaninit，因此把它设计成private static的
		...
		*/
		private static void keyBeanMemcpy (byte[] output, byte[] input, int outpos, int inpos, int len){
			int i;
			for (i = 0; i < len; i++)
