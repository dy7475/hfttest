...
public final class DateTimeHelper {
	...
	/**
	 * 把时间格式化成如：2002年07月05日 格式的字符串
	 */
	public final static String FMT_yyyyMMdd_1 = "yyyy年MM月dd日";
	
	/**
	 * 把时间格式化成如：2002/07/05 格式的字符串
	 */
	public final static String FMT_yyyyMMdd_2 = "yyyy/MM/dd";
	 ...
	 * @return 如果为null，返回字符串""
	 */
	public static String formatDateTimetoString(Object date, String formatStr) {
		if (date == null) {
			return null;
		}
		String reStr = "";
		if (date instanceof Date) {
			reStr = formatDateTimetoString((Date) date, formatStr);
		} else if (date instanceof String) {
			reStr = formatDateTimetoString(date.toString(), formatStr);
		}
		return reStr == null ? "" : reStr;
	}
	/**
  ...
  public static Date addTime(Date d, double times, int type) {
	...
	* @throws
	 */
	public static String getFullTimeStr(String time, boolean start){
	...
	public static Date[] getTodayTime(){
		...
		return new Date[]{calendar.getTime(), timeNow};
	}
	 ...
	 * @author 张宏利
	 * @since 2017年5月3日
	 * @param startDate 开始月份，结束月份以这个时间+1个月
	 * @return
	 */
	public static String[] getBetweenMonth(Date startDate){
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(startDate);
		calendar.set(Calendar.DAY_OF_MONTH, 1);
		String startMonth = DateTimeHelper.formatDatetoString(calendar.getTime());
		calendar.add(Calendar.MONTH, 1);
		String endMonth = DateTimeHelper.formatDatetoString(calendar.getTime());
	...
	public static List<String> createDateDic(String dateCountType, String dateRange) throws Exception {
		...
		if(dateArr.length==2){
			...
			if("0".equals(dateCountType)){
				return getBetweenDays(dateArr[0], dateArr[1]);
			}else if("1".equals(dateCountType)){//周统计
				return getBetweenWeeks(dateArr[0],dateArr[1],null);
			}else if("2".equals(dateCountType)){//月统计
				return getMonthBetween(dateArr[0], dateArr[1]);
			}
		}
		return null;
	}
	...
	public static Map<String,String> createDateDicOffConstomOffMap(String dateCountType, String dateRange,String customStr,boolean flag) throws Exception {
		...
		if (dateArr.length == 2) {
			// 日统计
			if ("0".equals(dateCountType)) {
				return getBetweenDaysOffMap(dateArr[0], dateArr[1], flag);
			} else if ("1".equals(dateCountType)) {// 周统计
				return getBetweenWeeksOffMap(dateArr[0], dateArr[1], customStr, flag);
			} else if ("2".equals(dateCountType)) {// 月统计
				return getMonthBetweenOffMap(dateArr[0], dateArr[1], flag);
			}
		}
		return null;
	}
	 ...
	 * @throws Exception
	 */
	public static Integer getBetweenDaysCount(String stime, String etime, boolean flag) {
		Integer count = 0;
		try {
			SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
			Date sdate = df.parse(stime);
	...
	public static List<String> getBetweenWeeks(String minDate,String maxDate,String customStr) {
		try {
			...
			minDate = DateTimeHelper.formatDateTimetoString(minDate, DateTimeHelper.formatDateTimetoString(minDate,DateTimeHelper.FMT_yyyyMMdd));
			maxDate = DateTimeHelper.formatDateTimetoString(minDate, DateTimeHelper.formatDateTimetoString(maxDate,DateTimeHelper.FMT_yyyyMMdd));
			if(weeks==0){
	...
	*/
	 ...
	 * @return java.util.Date类型对象
	 */
	public static Date parseToDateStr(String dateTimeStr){
		if (dateTimeStr == null || dateTimeStr.trim().length() < 1) {
			throw new IllegalArgumentException("参数dateTimeSt不能是null或空格串!");
		}
		int formatStrLength = formatStr.length;
	 ...
	 * @return
	 */
	public static String transferWeekDayOffCn(Date date){
	...
	*/
    ...
    /**
     * 获取某月最后一天的23:59:59
     * @param  "2019-04-18" or "2019-04"
     * @return "2019-04-30 23:59:59"
     */
