...
public class DOM4JHelper {
	...
	public static String getAttributeValue(Element el, String name) {
	 ...
	 * @throws Exception
	 */
	public static void saveFile(Document document, File file) throws Exception {
		// 紧凑的格式
	...
	public static Node getSingleNode(Element parent, String nodeNodeName) {
		...
		if (StringUtils.isBlank(nodeNodeName)) {
			return null;
		}
		xpath += nodeNodeName;
	...
	public static <T> T getNameNode(String xmlFilePath, String tagName, Flag flag) {
		...
		if (StringUtils.isBlank(tagName) || StringUtils.isBlank(xmlFilePath)) {
			return null;
		}
		Element rootElement = getRootNode(xmlFilePath);
		if (rootElement == null) {
	...
	public static <T> T getNameNodeAllAttributeMap(String xmlFilePath, String nodeName, Flag flag) {
		...
		if (StringUtils.isBlank(xmlFilePath) || StringUtils.isBlank(nodeName)) {
			return null;
		}
		switch (flag) {
		case one:
	...
	public static Element getSingleElement(Element root, String elName) {
		List<Element> kids = getChildList(root);
		for (Element kid : kids) {
			if (kid.getName().equalsIgnoreCase(elName)) {
				return kid;
			}
		}
		return null;
